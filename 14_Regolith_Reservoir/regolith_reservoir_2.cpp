...
*/
bool sandFlow(std::pair<int, int> initialPosition, std::pair<int, int> sourcePosition, std::vector<std::vector<char>> &grid, bool &firstTime){
    ...
    if(grid[initialPosition.first + 1][initialPosition.second] == '.')             // Check bottom position
        return sandFlow(std::make_pair(initialPosition.first + 1, initialPosition.second), sourcePosition, grid, firstTime);
    else if(grid[initialPosition.first + 1][initialPosition.second - 1] == '.')    // Check bottom-left position
        return sandFlow(std::make_pair(initialPosition.first + 1, initialPosition.second - 1), sourcePosition, grid, firstTime);
    else if(grid[initialPosition.first + 1][initialPosition.second + 1] == '.')    // Check bottom-right position
        return sandFlow(std::make_pair(initialPosition.first + 1, initialPosition.second + 1), sourcePosition, grid, firstTime);

    // Check if the next position to place a sandball would be the source
    if(initialPosition == sourcePosition && !firstTime) 
        return true;
...
int main(){
    ...
    // It's not the most elegant solution, but I established a margin and tried enlarging it until the result got stable
    int margin = 350;

    // This will be our reference
    int offset = minX - (margin/2);  // We establish a margin on each side. Substracting one will center the grid
    
   // Define the grid 
    std::vector<std::vector<char>> grid(high + 3, std::vector<char>(wide + margin)); // Added margins: one row down and one column on each side
    ...
    // Place the floor
    for(int i = 0; i < wide + margin; i++){
        grid[high + 1][i] = '#';
    }

    // Again, not the most elegant solution. Sorry :)
    bool firstTime = true;

    // Big loop
    int numBalls = 0;
    while(!sandFlow(sourcePosition, sourcePosition, grid, firstTime)) numBalls ++;

    // Log the results
    printGrid(grid);
    std::cout << "The result is " << numBalls + 1 << '\n';
